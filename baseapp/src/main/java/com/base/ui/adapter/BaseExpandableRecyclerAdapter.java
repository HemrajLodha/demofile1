...
import com.base.ui.adapter.model.ExpandableWrapper;
 ...
 */
public abstract class BaseExpandableRecyclerAdapter<P extends Parent<C, CF>, C, CF, PVH extends ParentViewHolder, CVH extends ChildViewHolder>
        extends RecyclerView.Adapter<RecyclerView.ViewHolder> {

    protected Context mContext;

    protected OnParentClickListener mOnParentClickListener;
    ...
    /***
     * default view for child footer
     */
    public static final int TYPE_CHILD_FOOTER = 3;
    /**
     ...
     * available in {@link BaseExpandableRecyclerAdapter}.
     */
    @NonNull
    protected List<ExpandableWrapper<P, C, CF>> mFlatItemList;
    ...
    public BaseExpandableRecyclerAdapter(Context context, OnParentClickListener onParentClickListener, OnChildClickListener onChildClickListener) {
        mContext = context;
    ...
    protected View getParentView(ViewGroup parent, int viewType) {
        return LayoutInflater.from(mContext).inflate(getParentResourceLayout(viewType), parent, false);
    }

    protected View getChildView(ViewGroup parent, int viewType) {
        return LayoutInflater.from(mContext).inflate(getChildResourceLayout(viewType), parent, false);
    }
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int flatPosition) {
        ...
        ExpandableWrapper<P, C, CF> listItem = mFlatItemList.get(flatPosition);
        if (listItem.isParent()) {
            ...
            parentViewHolder.mChildFooter = listItem.getParent().getChildListFooter();
            ...
            childViewHolder.mParent = mParentList.get(getNearestParentPosition(flatPosition));
            if (listItem.isChildFooter()) {
                childViewHolder.mChildFooter = mParentList.get(getNearestParentPosition(flatPosition)).getChildListFooter();
                onBindChildViewHolder(childViewHolder, getNearestParentPosition(flatPosition), getChildPosition(flatPosition),
                        mParentList.get(getNearestParentPosition(flatPosition)),
                        mParentList.get(getNearestParentPosition(flatPosition)).getChildListFooter());
            } else {
    ...
    protected abstract int getParentResourceLayout(int viewType);

    protected abstract int getChildResourceLayout(int viewType);
     ...
     */
    @UiThread
    public void onBindParentViewHolder(@NonNull PVH parentViewHolder, int parentPosition, @NonNull P parent) {
     ...
     */
    @UiThread
    public void onBindChildViewHolder(@NonNull CVH childViewHolder, int parentPosition, int childPosition, @Nullable P parent, @Nullable CF child) {
        childViewHolder.bind(child);
    }
    ...
    public int getItemViewType(int flatPosition) {
        ...
        if (listItem.isParent()) {
            ...
            if (listItem.isChildFooter()) {
                return getChildFooterViewType(getNearestParentPosition(flatPosition), getChildPosition(flatPosition));
            }
    ...
    /***
     * get default view type for child footer
     * @param parentPosition
     * @param childPosition
     * @return
     */
    public int getChildFooterViewType(int parentPosition, int childPosition) {
        return TYPE_CHILD_FOOTER;
    }
    ...
    /***
     * get single parent item
     * @param position
     * @return
     */
    public P getParentItem(int position) {
        if (position < mParentList.size()) {
     ...
     * <p>
     * Called when this {@link BaseExpandableRecyclerAdapter} is attached to a RecyclerView.
     *
    ...
    protected void parentExpandedFromViewHolder(int flatParentPosition) {
        ExpandableWrapper<P, C, CF> parentWrapper = mFlatItemList.get(flatParentPosition);
    ...
    public void expandParent(@NonNull P parent) {
        ExpandableWrapper<P, C, CF> parentWrapper = new ExpandableWrapper<>(parent);
     ...
     * {@link BaseExpandableRecyclerAdapter} is attached to.
     * <p>
    ...
    public void onRestoreInstanceState(@Nullable Bundle savedInstanceState) {
        ...
        List<ExpandableWrapper<P, C, CF>> itemList = new ArrayList<>();
        int parentsCount = mParentList.size();
        for (int i = 0; i < parentsCount; i++) {
            ExpandableWrapper<P, C, CF> parentWrapper = new ExpandableWrapper<>(mParentList.get(i));
            ...
            if (expandedStateMap.containsKey(i)) {
                ...
                if (expanded) {
                    List<ExpandableWrapper<P, C, CF>> wrappedChildList = parentWrapper.getWrappedChildList();
                    int childrenCount = wrappedChildList.size();
                    for (int j = 0; j < childrenCount; j++) {
                        ExpandableWrapper<P, C, CF> childWrapper = wrappedChildList.get(j);
    ...
    @SuppressWarnings("unchecked")
    @UiThread
    private void expandViews(@NonNull ExpandableWrapper<P, C, CF> parentWrapper, int flatParentPosition) {
        PVH viewHolder;
    ...
    @SuppressWarnings("unchecked")
    @UiThread
    private void collapseViews(@NonNull ExpandableWrapper<P, C, CF> parentWrapper, int flatParentPosition) {
        PVH viewHolder;
        for (RecyclerView recyclerView : mAttachedRecyclerViewPool) {
            viewHolder = (PVH) recyclerView.findViewHolderForAdapterPosition(flatParentPosition);
            if (viewHolder != null && viewHolder.isExpanded()) {
     ...
     */
    @UiThread
    private void updateExpandedParent(@NonNull ExpandableWrapper<P, C, CF> parentWrapper, int flatParentPosition, boolean expansionTriggeredByListItemClick) {
        ...
        if (wrappedChildList != null) {
            ...
            if (parentWrapper.getParent().expandableViewFooterEnable()) {
                mFlatItemList.add(flatParentPosition + childCount + 1, parentWrapper.getWrappedChildFooter());
                childCount++; // increase child count for child footer
            }
     ...
     */
    @UiThread
    private void updateCollapsedParent(@NonNull ExpandableWrapper<P, C, CF> parentWrapper, int flatParentPosition, boolean collapseTriggeredByListItemClick) {
        if (!parentWrapper.isExpanded()) {
            return;
        }
        ...
        if (wrappedChildList != null) {
            int childCount = wrappedChildList.size();
            if (parentWrapper.getParent().expandableViewFooterEnable()) {
                childCount++; // increase count if footer enabled
            }
    ...
    public int getNearestParentPosition(int flatPosition) {
        ...
        for (int i = 0; i <= flatPosition; i++) {
            ExpandableWrapper<P, C, CF> listItem = mFlatItemList.get(i);
            if (listItem.isParent()) {
                parentCount++;
            }
        }
    ...
    private int removeParentWrapper(int flatParentPosition) {
        int sizeChanged = 1;
        ExpandableWrapper<P, C, CF> parentWrapper = mFlatItemList.remove(flatParentPosition);
        if (parentWrapper.isExpanded()) {
    ...
    public void notifyParentMoved(int fromParentPosition, int toParentPosition) {
        ...
        ExpandableWrapper<P, C, CF> fromParentWrapper = mFlatItemList.get(fromFlatParentPosition);
        ...
        if (isCollapsed || isExpandedNoChildren) {
            ...
            ExpandableWrapper<P, C, CF> toParentWrapper = mFlatItemList.get(toFlatParentPosition);
            ...
            mFlatItemList.add(toFlatParentPosition + childOffset, fromParentWrapper);
            List<ExpandableWrapper<P, C, CF>> wrappedChildList = fromParentWrapper.getWrappedChildList();
    ...
    public void notifyChildInserted(int parentPosition, int childPosition) {
        ...
        if (parentWrapper.isExpanded()) {
            ExpandableWrapper<P, C, CF> child = parentWrapper.getWrappedChildList().get(childPosition);
    ...
    public void notifyChildRangeInserted(int parentPosition, int childPositionStart, int itemCount) {
        ...
        if (parentWrapper.isExpanded()) {
            List<ExpandableWrapper<P, C, CF>> wrappedChildList = parentWrapper.getWrappedChildList();
            for (int i = 0; i < itemCount; i++) {
                ExpandableWrapper<P, C, CF> child = wrappedChildList.get(childPositionStart + i);
    ...
    public void notifyChildRangeChanged(int parentPosition, int childPositionStart, int itemCount) {
        ...
        if (parentWrapper.isExpanded()) {
            ...
            for (int i = 0; i < itemCount; i++) {
                ExpandableWrapper<P, C, CF> child
    ...
    public void notifyChildMoved(int parentPosition, int fromChildPosition, int toChildPosition) {
        ...
        parentWrapper.setParent(parent);
        if (parentWrapper.isExpanded()) {
            ExpandableWrapper<P, C, CF> fromChild = mFlatItemList.remove(flatParentPosition + 1 + fromChildPosition);
     ...
     *                   the {@link BaseExpandableRecyclerAdapter}
     * @return A list of all parents and their children, expanded
     */
    private List<ExpandableWrapper<P, C, CF>> generateFlattenedParentChildList(List<P> parentList) {
        List<ExpandableWrapper<P, C, CF>> flatItemList = new ArrayList<>();

        int parentCount = parentList.size();
        for (int i = 0; i < parentCount; i++) {
            P parent = parentList.get(i);
            generateParentWrapper(flatItemList, parent, parent.isInitiallyExpanded(), parent.expandableViewFooterEnable());
        }
     ...
     */
    private List<ExpandableWrapper<P, C, CF>> generateFlattenedParentChildList(List<P> parentList, Map<P, Boolean> savedLastExpansionState) {
        List<ExpandableWrapper<P, C, CF>> flatItemList = new ArrayList<>();
        ...
        for (int i = 0; i < parentCount; i++) {
            ...
            generateParentWrapper(flatItemList, parent, shouldExpand, parent.expandableViewFooterEnable());
        }

        return flatItemList;
    }

    private void generateParentWrapper(List<ExpandableWrapper<P, C, CF>> flatItemList, P parent, boolean shouldExpand, boolean expandableViewFooterEnable) {
        ExpandableWrapper<P, C, CF> parentWrapper = new ExpandableWrapper<>(parent);
        flatItemList.add(parentWrapper);
        if (shouldExpand) {
            generateExpandedChildren(flatItemList, parentWrapper, expandableViewFooterEnable);
        }
    }

    private void generateExpandedChildren(List<ExpandableWrapper<P, C, CF>> flatItemList,
                                          ExpandableWrapper<P, C, CF> parentWrapper,
        ...
        if (expandableViewFooterEnable) {
            flatItemList.add(parentWrapper.getWrappedChildFooter()); // add child footer
        }
    }
    ...
    public interface OnParentClickListener<P> {
        void onParentClick(View view, P parent, int position);
    }

    public interface OnChildClickListener<P extends Parent, C> {
        void onChildClick(View view, P parent, C child);
    }
}
